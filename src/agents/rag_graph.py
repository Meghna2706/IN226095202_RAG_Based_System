...
def get_llm(cfg: dict = {}):
    ...
    if groq_key:
        ...
        return ChatGroq(model=cfg.get("llm_model", "llama3-8b-8192"), temperature=0)
    elif openai_key:
        from langchain_openai import ChatOpenAI
        return ChatOpenAI(model=cfg.get("llm_model", "gpt-3.5-turbo"), temperature=0)
    else:
    ...
    for kw in keywords:
        if kw in q:
            return {
                **state,
                "escalated": True,
                "escalation_reason": f"Sensitive keyword: '{kw}'",
                "answer": ""
            }
...
def retriever_node(state: SupportState, config: dict) -> SupportState:
    ...
    vs = get_vectorstore(
        cfg.get("chroma_dir", "./chroma_db"),
        cfg.get("collection_name", "customer_support_kb")
    )

    docs = vs.similarity_search(state["query"], k=cfg.get("top_k", 6))
...
def grader_node(state: SupportState, config: dict) -> SupportState:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         ...
         "Decide if the document is relevant to the question.\n"
    ...
    for doc in state["retrieved_docs"]:
        try:
            r = llm.invoke(prompt.format_messages(
                question=state["query"],
                document=doc
            ))

            if "YES" in r.content.upper():
                relevant.append(doc)
    ...
    if not relevant:
        relevant = state["retrieved_docs"][:2]
...
def generator_node(state: SupportState, config: dict) -> SupportState:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         ...
         "If exact info is not available, give the best possible helpful answer based on context.\n"
    ...
    try:
        ...
        answer = "I'm not sure about that. Let me connect you with support."

    # confidence heuristic
    low_phrases = ["not sure", "don't know", "cannot find"]
    confidence = 0.5 if any(p in answer.lower() for p in low_phrases) else 0.9
...
def hitl_node(state: SupportState, config: dict) -> SupportState:
    msg = (
        "Thank you for reaching out! Your question needs attention from a specialist. "
...
def route_after_intent(state):
    return "hitl" if state.get("escalated") else "retriever"
